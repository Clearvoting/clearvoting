...
function renderMember(container, member, bioguideId) {
    ...
    // Voting summary (At a Glance card, loaded async)
    const summaryEl = el('div', { id: 'voting-summary', className: 'voting-summary', 'aria-live': 'polite' });
    container.appendChild(summaryEl);

    // Visualization sections (donut, attendance, topics — above tab bar)
    const vizEl = el('div', { id: 'voting-viz' });
...
async function loadVotingRecord(bioguideId) {
    ...
    try {
        ...
        clearEl(container);
        _congressesCache = data.congresses || [];
        renderVotingSummary(data.stats, data.votes, summaryData);
        renderVotingStats(container, data.stats, data.congresses, data.categories || data.policy_areas, data.votes, summaryData);
...
function renderVotingSummary(stats, votes, summaryData) {
    ...
    const yeaPct = stats.yea_count + stats.nay_count > 0
        ? Math.round((stats.yea_count / (stats.yea_count + stats.nay_count)) * 100)
        : 0;
    ...
    // At a Glance card with narrative, stats dashboard, key bills
    const card = el('section', { className: 'glance-card' });
    card.appendChild(el('h2', null, 'At a Glance'));

    // AI narrative
    if (summaryData && summaryData.narrative) {
        const narrative = el('p', { className: 'narrative' }, summaryData.narrative);
        narrative.style.fontFamily = 'var(--font-narrative)';
    ...
    const Viz = window.ClearVoteViz;
    const dashboard = el('div', { className: 'viz-stats-dashboard' });

    // Compute congress range for detail text
    let congressDetail = '';
    if (_congressesCache && _congressesCache.length > 1) {
        congressDetail = `${Math.min(..._congressesCache)}th\u2013${Math.max(..._congressesCache)}th Congress`;
    } else if (_congressesCache && _congressesCache.length === 1) {
        congressDetail = `${_congressesCache[0]}th Congress`;
    }

    dashboard.appendChild(Viz.createStatCard('Total Votes', stats.total_votes.toLocaleString(), congressDetail));
    dashboard.appendChild(Viz.createStatCard('Participation', `${stats.participation_rate}%`, 'of all floor votes'));
    dashboard.appendChild(Viz.createStatCard('Support Rate', `${yeaPct}%`, 'voted yea'));
    card.appendChild(dashboard);

    // Key Bills Supported
    if (uniqueYea.length > 0) {
        const supSection = el('div', { className: 'viz-key-bills-section' });
        supSection.appendChild(el('h3', { className: 'viz-key-bills-heading viz-supported-heading' }, 'Key Bills Supported'));
        const supList = el('ul', { className: 'viz-key-bills-list' });
        uniqueYea.forEach(v => supList.appendChild(el('li', null, v.one_liner)));
        supSection.appendChild(supList);
        card.appendChild(supSection);
    }

    // Key Bills Opposed
    if (uniqueNay.length > 0) {
        const oppSection = el('div', { className: 'viz-key-bills-section' });
        oppSection.appendChild(el('h3', { className: 'viz-key-bills-heading viz-opposed-heading' }, 'Key Bills Opposed'));
        const oppList = el('ul', { className: 'viz-key-bills-list' });
        uniqueNay.forEach(v => oppList.appendChild(el('li', null, v.one_liner)));
        oppSection.appendChild(oppList);
...
function renderVotingStats(container, stats, congresses, categories, votes, summaryData) {
    allVotes = votes;
    ...
    // Render viz sections above the tab bar (into #voting-viz)
    const vizContainer = document.getElementById('voting-viz');
    if (vizContainer) {
        clearEl(vizContainer);

        // Vote Breakdown donut chart
        const donutSection = el('section', { className: 'viz-section' });
        donutSection.appendChild(el('h3', null, 'Vote Breakdown'));
        const donutChart = Viz.createDonutChart([
            { label: 'Yea / Aye', count: stats.yea_count, color: 'var(--vote-yea)' },
            { label: 'Nay / No', count: stats.nay_count, color: 'var(--vote-nay)' },
            { label: 'Not Voting', count: stats.not_voting_count, color: 'var(--vote-absent)' },
        ], total, 160);
        donutSection.appendChild(donutChart);
        vizContainer.appendChild(donutSection);

        // Attendance bar
        const attendanceSection = el('section', { className: 'viz-section' });
        attendanceSection.appendChild(el('h3', null, 'Attendance'));
        const attended = Math.round(total * stats.participation_rate / 100);
        attendanceSection.appendChild(Viz.createAttendanceBar(
            stats.participation_rate, attended, total, congressLabel
        ));
        vizContainer.appendChild(attendanceSection);

        // Topic breakdown — from summaryData.top_policy_areas
        const topicAreas = (summaryData && summaryData.top_policy_areas) || [];
        if (topicAreas.length > 0) {
            const topicSection = el('section', { className: 'viz-section' });
            topicSection.appendChild(el('h3', null, 'What They Fight For by Topic'));

            const subtext = el('p', { className: 'viz-topic-subtext' });
            subtext.appendChild(document.createTextNode('Each row shows how many bills this representative '));
            const strongSup = el('strong', null, 'supported');
            strongSup.style.color = 'var(--vote-yea)';
            subtext.appendChild(strongSup);
            subtext.appendChild(document.createTextNode(' vs. '));
            const strongOpp = el('strong', null, 'opposed');
            strongOpp.style.color = 'var(--vote-nay)';
            subtext.appendChild(strongOpp);
            subtext.appendChild(document.createTextNode(' in that topic area.'));
            topicSection.appendChild(subtext);

            const bars = el('div', { className: 'viz-topic-bars' });
            topicAreas.forEach(area => {
                bars.appendChild(Viz.createTopicBar(area.in_favor, area.against, area.name));
            });
            topicSection.appendChild(bars);
            vizContainer.appendChild(topicSection);
        }
    }

    // Filter dropdowns row (stays inside voting-record tab)
    const filterRow = el('div', { className: 'filter-row' });

    // Vote type dropdown
    const voteTypeWrap = el('div', { className: 'filter-wrap' });
    ...
    voteTypeWrap.appendChild(el('span', { className: 'filter-arrow' }, '\u25BC'));
    filterRow.appendChild(voteTypeWrap);

    // Category dropdown
    const categoryWrap = el('div', { className: 'filter-wrap' });
    ...
    categoryWrap.appendChild(el('span', { className: 'filter-arrow' }, '\u25BC'));
